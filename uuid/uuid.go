...
package uuid // import "github.com/jet/go-mantis/uuid"
...
func (g *rfc4122Generator) tick() ([6]byte, uint64, uint16, error) {
	...
	g.initOnce.Do(func() {
		...
		g.hwAddr = hwAddr
		cbs := make([]byte, 2)
		if _, e := rand.Read(cbs); e != nil {
			err = fmt.Errorf("uuid: init clock seq failed: %v", e)
		}
...
func (u UUID) Format(s fmt.State, verb rune) {
	...
	case 's':
		switch {
		case s.Flag('+'):
			fmt.Fprintf(s, strings.ToUpper(u.String()))
		default:
			fmt.Fprintf(s, u.String())
		}
	case 'x':
		fmt.Fprintf(s, hex.EncodeToString(u[:]))
	case 'X':
		fmt.Fprintf(s, strings.ToUpper(hex.EncodeToString(u[:])))
	case 'v':
		u.Format(s, 's')
	case 'q':
		fmt.Fprintf(s, `"`)
